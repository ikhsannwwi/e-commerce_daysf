                            ...
                            <div class="col-6">
                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">Kategori Set</li>
                                    </ol>
                                </nav>
                            </div>
                            <div class="col-6">
                                @if (isallowed('kategori_set', 'add'))
                                    <a href="{{ route('admin.kategori_set.add') }}" class="btn btn-primary me-3 float-end">Tambah
                                        Data</a>
                                @endif
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var data_table = $('#datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('admin.kategori_set.getData') }}',
                    dataType: "JSON",
                    type: "GET",
                },
                columns: [{
            ...
            $(document).on('click', '.delete', function(event) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('admin.kategori_set.delete') }}",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "_method": "DELETE",
                                "id": id,
